...
class LlamaTurnPredictor:
    ...
    def predict_trp(self, text: str, vap_prediction: bool, vap_confidence: float) -> tuple[
        bool, float, float, tuple[float, float]]:
        start_time = time.time()
        ...
        messages = [
            ...
            {"role": "user",
             "content": f"Speech segment: {text}\nVAP confidence: {1 - vap_confidence:.3f} (threshold: 0.9, prediction: {'yes' if vap_confidence <= 0.1 else 'no'})"}
        ]
    ...
    def process_stream(self, audio_path: str, vap_results: dict, chunk_size=1.0):
        ...
        frames_per_chunk = int(50 * chunk_size)
        ...
        for i in range(0, len(audio_array), chunk_samples):
            ...
            try:
                frame_start = int(i * 50 / 16000)
                frame_end = frame_start + frames_per_chunk
                vap_pred = vap_results['predictions'][frame_start:frame_end]
                vap_prob = vap_results['probabilities'][frame_start:frame_end]

                vap_confidence = np.mean(vap_prob)

                text, asr_time = self.transcribe_chunk(chunk)
                has_trp, confidence, llm_time, (yes_prob, no_prob) = self.predict_trp(
                    text, vap_confidence <= 0.1, vap_confidence
                )
        ...
        frame_predictions = np.repeat(predictions, frames_per_chunk)
        frame_probabilities = np.repeat(probabilities, frames_per_chunk)
...
class LlamaTurnEvaluator(TurnShiftEvaluator):
    ...
    def evaluate_file(self, audio_path, ground_truth_data, flip_predictions=False):
        ...
        if not hasattr(self.model, 'vap_results') or audio_file not in self.model.vap_results:
            raise ValueError(f"VAP results not found for {audio_file}")

        predictions, probabilities, rtf = self.model.process_stream(
            audio_path,
            self.model.vap_results[audio_file]
        )
...
def main():
    ...
    vap_results_dir = Path(
        "C:/Users/Harry/PycharmProjects/LLA-VAP/prediction_results/baseline_VAP_CCPE_output/0.1_flip")
    output_dir = Path("C:/Users/Harry/PycharmProjects/LLA-VAP/output")
    vap_results = {}

    # Load VAP results from CSV files
    for csv_file in vap_results_dir.glob("*_probabilities.csv"):
        audio_file = csv_file.stem.replace("_probabilities", "")
        df = pd.read_csv(csv_file)
        vap_results[audio_file] = {
    ...
    evaluator = LlamaTurnEvaluator(threshold=0.4, flip_predictions=False)

    # Need to modify LlamaTurnEvaluator.evaluate_file to accept vap_results
    evaluator.model.vap_results = vap_results  # Store vap_results in the model instance
    results = evaluator.evaluate_directory(audio_dir, ground_truth, output_dir)
    ...
    if results:
        ...
        print(f"Mean Real-time Factor: {mean_metrics['real_time_factor']:.3f}")
